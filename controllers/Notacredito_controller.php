<?php
class Notacredito_controller extends Controller {
    ...
    public function agregar() {
        ...
        $nota_pedidos = Pedidosnotacredito::whereV($data_pedidos, 'and');
        if(empty($nota_pedidos)){
            echo 3;
            exit;
        }
        $nota                     = new Notacredito($id, $nota_fecha, $nota_correlativo, $nota_numero, $nota_moneda, $nota_tipocambio, $nota_sunat_transaccion, $nota_fecha_vencimiento, $nota_cli_id, $nota_documento_modificar, $nota_numero_modificar, $nota_tipo, $nota_correlativo_numero, $nota_envio_sunat, $nota_pdf, $nota_xml, $nota_cdr);
        $nota->create();
       ...
       if (isset($datos_recibidos['aceptada_por_sunat'])){
           if(empty($datos_recibidos['enlace_del_cdr'])){
               ...
               $nota_credito = Notacredito::getByData($data, 'and');
    ...
    public function llamar_cod() {
        ...
        $pedidos_doc = Notacredito::whereV($data, 'and', 'nota_numero');
    ...
    public function traer_datos_item_nota() {
        ...
        $datos   = array();
        $pedidos = Pedidosnotacredito::whereV($data, 'and');
        foreach ($pedidos as $value) {
            array_push($datos, array(
                'id' => $value['id'],
                'ped_cantidad' => $value['ped_cantidad'],
    ...
    public function guardar_items() {
        ...
        $nota_credito_pedido     = new Pedidosnotacredito($id, $ped_correlativo, $ped_numero, $ped_id_pro, $ped_cod_pro, $ped_nombre_pro, $ped_cantidad, $ped_valor_venta_sin_igv, $ped_total);
    ...
    public function datos_json($correlativo = '', $numero = '') {
        ...
        $nota_doc     = Notacredito::whereV($data_doc, 'and');
        $nota_pedidos = Pedidosnotacredito::whereV($data_pedidos, 'and');
        ...
        $datos_json = array(
            ...
            'tipo_de_nota_de_credito' => $nota_doc[0]['nota_tipo'],
     ...
     public function eliminar() {
        $id        = $_POST['id'];
        $notacredito = Pedidosnotacredito::getById($id);
        $notacredito->delete();
    }
}