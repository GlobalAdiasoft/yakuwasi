 $(document).ready(function() {
     ...
     $("#form_agregar input[name=nombre]").keyup(function() {
         ...
         $("#form_agregar input[name=codinterno]").val(letras + numeros + '_mar');
     });
     ...
     $('.datatable tbody').on('click', '#btn_eliminar', function() {
         ...
         if (data == undefined) {
             ...
             alertify.confirm('<i class="fa fa-bars" aria-hidden="true"></i> Eliminar', '¿Desea eliminar la familia <strong>' + rowData['mar_nombre'] + '</strong>?',
                 function() {
                     eliminar(rowData['id']);
                     alertify.notify('Se elimino la marca <strong>' + rowData['mar_nombre'] + '</strong> correctamente.', 'custom-black', 4, function() {});
                 },
                 function() {
                     alertify.notify('Se cancelo la <strong>eliminación</strong>.', 'custom-black', 4, function() {});
                 }).set('labels', { ok: 'Eliminar', cancel: 'Cancelar' });
         } else {
             alertify.confirm('<i class="fa fa-bars" aria-hidden="true"></i> Eliminar', '¿Desea eliminar la marca <strong>' + data['mar_nombre'] + '</strong>?',
                 function() {
                     eliminar(data['id']);
                     alertify.notify('Se elimino la marca <strong>' + data['mar_nombre'] + '</strong> correctamente.', 'custom-black', 4, function() {});
                 },
                 function() {
                     alertify.notify('Se cancelo la <strong>eliminación</strong>.', 'custom-black', 4, function() {});
                 }).set('labels', { ok: 'Eliminar', cancel: 'Cancelar' });
     ...
     abrir_modal('modalmarca', '<i class="fa fa-bars" aria-hidden="true"></i> Modificar Marca');
 });
 $(function() {
 ...
 function editar(id) {
     $('#form_modificar').show();
     alertify.modalmarca($('#form_modificar')[0]).set('selector', 'input[name="codinterno"]');
     $.ajax({
         url: '../Marcas/mostrar_marca',
         type: "POST",
         dataType: "json",
         data: {
             'id': id,
         ...
         success: function(data) {
             ...
             form.find('input[name=codinterno]').val(data[0].mar_codinterno);
             form.find('input[name=nombre]').val(data[0].mar_nombre);
         }
     });
 }
 $(function() {
     $("#form_modificar").on("submit", function(e) {
         ...
         $.ajax({
             ...
             success: function(response) {
                 ...
                 table = $('.datatable').DataTable();
                 table.ajax.reload();
                 alertify.modalmarca().close();
                 alertify.notify('<strong>Marca</strong> modificada correctamente.', 'custom-black', 3, function() {});
             },
             error: function() {},
         });
     });
