$(document).ready(function() {
    ...
    var table = $('.datatable').DataTable({
        ...
        "info": false,
        "ajax": {
            "url": '../Notacredito/traer_datos_item_nota',
            "dataSrc": "",
            "data": function(d) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data == 0) {
                ...
                $('input[name=valor_venta_sin_igv]').val(data[0].art_precio_ventaconigv);
...
function traer_datos_factura() {
    ...
    if (numero == '') {
        ...
        $.ajax({
            url: '../Notacredito/traer_datos_factura',
            type: "POST",
            dataType: "json",
            data: {
                'documento': documento,
...
function abre_ventana() {
    ...
    if (numero == '') {
        ...
        if ($('input[name=numero]').val() == '') {
            ...
            ventana = window.open('../Index/facturacionelectronica_nota_credito_tabla?documento=' + documento + '&numero=' + numero, "mywindow", 'toolbar=0,location=0,status=0,menubar=0,scrollbars=1,resizable=0,width=1000,height=650');
            ventana.focus();
        }
    }
...
function agregaritems() {
    ...
    if (correlativo == null) {
        ...
        $.ajax({
            url: '../Notacredito/guardar_items_directo/' + codpro + '|' + correlativo + '|' + numero + '|' + producto + '|' + cantidad + '|',
            type: 'POST',
...
$(function() {
    $("#form_agregar").on("submit", function(e) {
        ...
        $.ajax({
            ...
            processData: false,
            beforeSend: function() {
                alertify.notify(' <img class="loading" src="../public/img/spinner.svg"> Enviando <strong>Nota de Crédito</strong> a <img class="alertify-sunat" src="../public/img/sunat.png" alt="">', 'custom-black', 3600, function() {});

            },
            success: function(response) {
                alertify.dismissAll();
                if (response == 1) {
                    alertify.notify('<strong>Nota de Crédito</strong> enviada correctamente', 'custom-black', 5, function() {});
                    f[0].reset();

                }
                if (response == 0) {
                    alertify.notify('<strong>Nota de Crédito</strong> guardada en base de datos pero no enviada', 'custom-black', 5, function() {});
                    f[0].reset();

                }
                if (response == 3) {
                    alertify.notify('<strong>Nota de Crédito</strong> debete tener item de especificación ', 'custom', 5, function() {});
