...
function verificarnumerofactura() {
    ...
    $.ajax({
        url: '../Ventas/verificarnumeroboleta',
        type: "POST",
        dataType: "html",
        data: {
...
function facturar() {
    if ($('input[name=total_pago]').val() == '' || $('input[name=total_pago]').val() == '0' || $('input[name=total_pago]').val() == '0.00') {
        ...
        var metodo = f.attr("method");
        var url = '../Boletear/crear_boleta2';
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                alertify.dismissAll();
                if (data == 0) {
                    alertify.notify('<strong>Boleta</strong> enviada correctamente', 'custom-black', 1, function() {
                        location.reload();
                    });
...
function paquetes(codigo) {
    $.ajax({
        ...
        success: function(data) {
            $(".select_paquetes tr").each(function() {
                $(this).remove();
            });
            $(".select_paquetes").append('<tr><th><small><b>Descripción</b></small></th><th><small><b>Cantidad</b></small></th><th><small><b>Precio</b></small></th><th><small><b>Precio</b></small></th></tr>');
            $.each(data, function(i, item) {
                $('.select_paquetes').append('<tr><td><small>' + item.descripcion + '</small></td><td><small>' + item.cantidad + '</small></td><td><small>' + item.precio + '</small></td><td><button type="button" class="btn btn-sm btn-info" onclick="precios_paquetes(' + item.cantidad + ',' + item.id_articulo + ')">Seleccionar</button></td></tr>');
            });
        }
    });
}

function precios_paquetes(cantidad, articulo) {
    $.ajax({
        url: '../Paquetes/precio/' + cantidad + '/' + articulo,
        type: "POST",
        dataType: "json",
        success: function(data) {
            console.log(data);
            $('input[name=cantidad]').val(data.cantidad);
            $('input[name=valor_venta_sin_igv]').val(data.precio);
            $('input[name=total]').val(data.precio);
            $('input[name=nombre_paquete]').val(data.descripcion.toUpperCase());
        }
    });
};
...
function listas() {
    $.ajax({
        ...
        beforeSend: function() {},
        success: function(data) {
            $(".select_horarios option").each(function() {
                $(this).remove();
            });
            $('.select_horarios').append('<option value="" disabled selected>Seleccione un horario</option>');
            $.each(data, function(i, item) {
                $('.select_horarios').append('<option value="' + item.id + '" >' + item.horario.toUpperCase() + '</option>');
            });
        },
    });
}