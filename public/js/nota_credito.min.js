...
});$("input[name=codproducto]").change(function(){cod_producto($(this).val());});var a=$(".datatable").DataTable({searching:false,bLengthChange:false,paging:false,info:false,ajax:{url:"../Notacredito/traer_datos_item_nota",dataSrc:"",data:function(b){b.correlativo=$("select[name=correlativo]").val();
...
$("#cli_direccion").text(c[0].cli_direccion);$("#cli_telefono").text(c[0].cli_telefono);$("#cli_celular").text(c[0].cli_celular);}}});}function llamar_cod(a){$.ajax({url:"../Notacredito/llamar_cod",type:"POST",dataType:"html",data:{documento:a,},success:function(c){$("input[name=numero]").val(c);
...
$("input[name=preciounitario]").val("");$("input[name=total]").val("");}else{$("input[name=producto]").val(f[0].art_nombre);$("input[name=valor_venta_sin_igv]").val(f[0].art_precio_ventaconigv);
$("input[name=pro_id]").val(f[0].id);var d=$("input[name=cantidad]").val();var g=$("input[name=valor_venta_sin_igv]").val();var e=d*g;$("input[name=total]").val(e);
...
}else{$.ajax({url:"../Notacredito/traer_datos_factura",type:"POST",dataType:"json",data:{documento:documento,numero:numero,},success:function(a){if(a.length>0){if(documento==1){$("input[name=cli_id]").val(a[0].fac_cli_id);
...
return;}else{ventana=window.open("../Index/facturacionelectronica_nota_credito_tabla?documento="+documento+"&numero="+numero,"mywindow","toolbar=0,location=0,status=0,menubar=0,scrollbars=1,resizable=0,width=1000,height=650");
ventana.focus();}}}function actualizar(){table=$(".datatable").DataTable();table.ajax.reload();}function agregaritems(){var e=$("#form_agregar");var c=e.find("input[name=codproducto]").val();
var g=e.find("select[name=correlativo]").val();var b=e.find("input[name=numero]").val();var a=e.find("input[name=producto]").val();var d=e.find("input[name=cantidad]").val();
if(g==null){e.find("select[name=correlativo]").focus();}else{if(e.find("input[name=numero_modi]").val()==""){e.find("input[name=numero_modi]").focus();
}else{if(c==""){e.find("input[name=codproducto]").focus();}else{if(e.find("input[name=cantidad]").val()==""||e.find("input[name=cantidad]").val()==0){e.find("input[name=cantidad]").focus();
}else{$.ajax({url:"../Notacredito/guardar_items_directo/"+c+"|"+g+"|"+b+"|"+a+"|"+d+"|",type:"POST",cache:false,processData:false,beforeSend:function(){},success:function(f){console.log(f);
...
e.find("input[name=codproducto]").focus();e.find("input[name=valor_venta_sin_igv]").val("");},error:function(){},});}}}}}$(function(){$("#form_agregar").on("submit",function(g){g.preventDefault();
var c=$(this);var b=c.attr("method");var a=c.attr("action");var d=new FormData(this);d.append("dato","valor");$.ajax({url:a,type:b,dataType:"html",data:d,cache:false,contentType:false,processData:false,beforeSend:function(){alertify.notify(' <img class="loading" src="../public/img/spinner.svg"> Enviando <strong>Nota de Crédito</strong> a <img class="alertify-sunat" src="../public/img/sunat.png" alt="">',"custom-black",3600,function(){});
},success:function(e){alertify.dismissAll();if(e==1){alertify.notify("<strong>Nota de Crédito</strong> enviada correctamente","custom-black",5,function(){});
c[0].reset();}if(e==0){alertify.notify("<strong>Nota de Crédito</strong> guardada en base de datos pero no enviada","custom-black",5,function(){});c[0].reset();
}if(e==3){alertify.notify("<strong>Nota de Crédito</strong> debete tener item de especificación ","custom",5,function(){});}},error:function(){},});});
});function eliminar(a){$.ajax({url:"../Notacredito/eliminar",type:"POST",dataType:"html",data:{id:a,},success:function(b){table=$(".datatable").DataTable();
table.ajax.reload();}});}